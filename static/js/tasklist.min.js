...
// console.log(...args);
}class TaskPlot extends React.PureComponent{constructor(props){super(props)}componentDidMount(){debug_log("activating plot",this.props.taskid);const plot_url=new URL(this.props.taskurl);plot_url.pathname+="resultplotdata.js";plot_url.search="";$.ajax({url:plot_url,cache:true,dataType:"script"})}componentWillUnmount(){debug_log("Unmounting plot for task ",this.props.taskid);delete jslimitsglobal["#plotforcedflux-task-"+this.props.taskid];delete jslcdataglobal["#plotforcedflux-task-"+this.props.taskid];delete jslabelsglobal["#plotforcedflux-task-"+this.props.taskid]}render(){return/*#__PURE__*/React.createElement("div",{key:"plot",id:"plotforcedflux-task-"+this.props.taskid,className:"plot",style:{width:"100%",height:"300px"}})}}export class Task extends React.Component{constructor(props){super(props);this.state={};this.state.updateTimeElapsed=this.updateTimeElapsed.bind(this);this.state.interval=null;this.state.timeelapsed=-1;this.state.httperror=""}deleteTask(){const li_id="#task-"+this.props.taskdata.id;// $(li_id).hide(300);
$(li_id).slideUp(200);setTimeout(()=>{$.ajax({headers:{"X-CSRFToken":getCookie("csrftoken")},url:this.props.taskdata.url,method:"delete",success:result=>{console.log("Deleted task ",this.props.taskdata.id);this.props.fetchData()},error:err=>{console.log("Failed to delete task ",this.props.taskdata.id,err);$("#task-"+this.props.taskdata.id).slideDown(100);this.props.fetchData()}})},200)}requestImages(){const request_image_url=new URL(this.props.taskdata.url);// the trailing slash matters: without it APPEND_SLASH answers with a 301, and a browser
...
const page_url=new URL(strurl);const started=page_url.searchParams.get("started");if(filtername==null){if(started==null&&this.singleTaskViewTaskId(this.state.dataurl)==null){return"btn-primary"}else{return"btn-link"}}else if(filtername=="started"){if(started=="true"){return"btn-primary"}else{return"btn-link"}}}setFilter(filtername){debug_log("changed filter to",filtername);const new_page_url=new URL(api_url_base);new_page_url.search="";if(filtername!=null){new_page_url.searchParams.set(filtername,true)}if(new_page_url!=window.location.href){window.history.pushState({},document.title,new_page_url);const statechanges={"scrollToTopAfterUpdate":true,dataurl:new_page_url};if(filtername=="started"&&this.state.results!=null){statechanges["results"]=this.state.results.filter(task=>{return task.starttimestamp!=null});if(statechanges["results"].length==0){// prevent flash of "there are no results" for empty ([] non-null) results list
