...
def do_taskloop():
    ...
    if not task_exists(taskid=task.id):  # task was cancelled
        ...
        remove_task_resultfiles(taskid=task.id, parent_task_id=task.parent_task_id, request_type=task.request_type)
    else:
        ...
        if error_msg:
            ...
            if localresultfile and os.path.exists(localresultfile):
                ...
                log(
                    logprefix
                    + f"ERROR: Task was not completed successfully. Waiting {waittime} seconds "
                    "before continuing with next task..."
                )
...
def remove_old_tasks(logprefix, days_ago, harddeleterecord=False, request_type=None, is_archived=None):
    ...
    log(
        logprefix
        + f"There are {taskcount} {strarchived}{strrequesttype}tasks that finished more than {days_ago} days ago"
    )
